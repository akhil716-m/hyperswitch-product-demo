    ...
    <div
      className={`${sizeClasses[size]} rounded-full flex items-center justify-center font-bold transition-all duration-500 ${
        isEliminated 
          ...
          : isSelected 
          ? 'bg-[#5e6ad2] text-white ring-4 ring-[#eef0fc]' 
...
const StatusBadge = ({ status, text }) => {
  const styles = {
    eligible: 'bg-[#eef0fc] text-[#5e6ad2] border-[#5e6ad2]',
    selected: 'bg-[#5e6ad2] text-white border-[#5e6ad2]',
...
const TransactionCard = ({ txn }) => {
  ...
  return (
    <div className="bg-white rounded-xl border-2 border-[#eef0fc] p-4 mb-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-12 h-8 bg-gradient-to-r from-[#5e6ad2] to-[#7b85e6] rounded-lg flex items-center justify-center">
        ...
        <div className="text-right">
          <span className="text-2xl font-bold text-[#5e6ad2] block">{txn.amountDisplay}</span>
...
const Step1Eligibility = ({ data, psps }) => {
  return (
    <div className="bg-[#eef0fc] rounded-2xl border-2 border-[#5e6ad2] p-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 rounded-full bg-[#5e6ad2] flex items-center justify-center shadow-lg">
          <Zap className="w-6 h-6 text-white" />
        </div>
        <div>
          <h4 className="text-xl font-bold text-[#5e6ad2]">Step 1: Eligibility Analysis</h4>
      ...
      <div className="bg-white rounded-xl border border-[#eef0fc] p-4">
        ...
        <div className="space-y-2">
          {data.outputPsps.map((psp) => (
            <div 
              key={psp.id} 
              className="flex items-center justify-between p-3 rounded-lg bg-[#eef0fc] border border-[#5e6ad2]"
            >
              <div className="flex items-center gap-3">
                <PspAvatar psp={psp} size="sm" isSelected={true} />
                <div>
                  <p className="font-bold text-gray-900">{psp.name}</p>
                  <p className="text-xs text-[#5e6ad2]">✓ Supports all transaction criteria</p>
                </div>
              </div>
      ...
      <div className="mt-4 p-3 bg-white rounded-lg border border-[#eef0fc]">
        <p className="text-sm text-gray-700">
          <span className="font-bold text-[#5e6ad2]">Result:</span> {data.outputPsps.length} of {psps.length} PSPs are eligible for this transaction
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#eef0fc] rounded-2xl border-2 border-[#5e6ad2] p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h4 className="text-xl font-bold text-[#5e6ad2]">Step 2: Rule-Based Routing</h4>
      ...
      {data.selectedPsp && (
        <div className="bg-white rounded-xl p-5 border-2 border-[#5e6ad2] shadow-md mb-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 pt-4 border-t border-gray-100">
            <div className="text-center">
              <p className="text-2xl font-bold text-[#5e6ad2]">{(data.selectedPsp.authRate * 100).toFixed(0)}%</p>
              <p className="text-xs text-gray-500 uppercase tracking-wide">Auth Rate</p>
            </div>
            <div className="text-center border-l border-gray-200">
              <p className="text-2xl font-bold text-[#5e6ad2]">{(data.selectedPsp.costPerTxn * 100).toFixed(1)}%</p>
              <p className="text-xs text-gray-500 uppercase tracking-wide">Cost</p>
            </div>
            <div className="text-center border-l border-gray-200">
              <p className="text-2xl font-bold text-[#5e6ad2]">{data.selectedPsp.avgLatency}ms</p>
      ...
      {data.rules && data.rules.length > 0 && (
        <div className="bg-white rounded-xl p-4 border border-[#eef0fc] mb-4">
          <p className="text-xs font-bold text-gray-500 mb-3 uppercase tracking-wide">Applied Rules</p>
          <div className="space-y-2">
            {data.rules.map((rule, idx) => (
              <div key={idx} className="flex items-start gap-2 p-2 bg-[#eef0fc] rounded-lg">
                <div className="w-6 h-6 rounded-full bg-[#5e6ad2] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                  {idx + 1}
                </div>
                <div>
      ...
      {data.availablePsps && data.availablePsps.length > 1 && (
        <div className="bg-white rounded-xl p-4 border border-[#eef0fc]">
  ...
  return (
    <div className="bg-[#eef0fc] rounded-2xl border-2 border-[#5e6ad2] p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h4 className="text-xl font-bold text-[#5e6ad2]">Step 3: Real-Time Override Check</h4>
      ...
      {data.wasOverridden ? (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl p-5 border border-[#eef0fc]">
            ...
            <div className="flex items-center justify-center gap-6">
              ...
              <div className="flex flex-col items-center">
                <ArrowRight className="w-8 h-8 text-[#5e6ad2]" />
                <span className="text-xs font-bold text-[#5e6ad2] mt-1">OVERRIDE</span>
              </div>
...
const TransactionFunnel = ({ currentTxn, psps }) => {
  if (!currentTxn) {
    return (
      <div className="bg-[#eef0fc] border-2 border-dashed border-[#5e6ad2] rounded-2xl p-12 text-center">
        <div className="w-20 h-20 rounded-full bg-[#5e6ad2] flex items-center justify-center mx-auto mb-4 shadow-lg">
          <Zap className="w-10 h-10 text-white" />
        </div>
        <h3 className="text-xl font-bold text-[#5e6ad2] mb-2">Ready to Route Transactions</h3>
      ...
      <div className={`rounded-2xl p-6 text-center border-2 ${
        ...
        {finalPsp && (
          <p className="text-gray-700">
            Processed via <span className="font-bold text-[#5e6ad2]">{finalPsp.name}</span> • 
